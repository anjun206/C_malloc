 ...
 * 
 * ++ 최적화를 위해 가용에만 푸터
 * 이전 prev_alloc으로 기록해 그 전 블록 이어 기록
 */
static void place(void *bp, size_t asize) {
    ...
    if (remainder >= MIN_BLOCK) {
        ...
        PUT(HDRP(NEXT_BLKP(bp)), nh | 0x2);                         // 다음 헤더에 prev_alloc=1만 OR
    }
}
...
static void *coalesce(void *bp)
{
    ...
    // case 2  다음 블록 가용가능
            ...
            PUT(FTRP(bp), PACK(size, 0));
            unsigned int nh = GET(HDRP(NEXT_BLKP(bp)));            // 병합 결과 앞=가용이므로
            PUT(HDRP(NEXT_BLKP(bp)), nh & ~0x2);                   // 다음 헤더 prev_alloc=0
        }
