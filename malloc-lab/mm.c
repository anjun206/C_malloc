...
int mm_init(void)
{
    if ((mem_heap = sbrk(4*WSIZE)) == (void *)-1)
        return -1;
    PUT(mem_heap, 0);
    PUT(mem_heap + (1*WSIZE), PACK(DSIZE, 1));  // 프롤로그 헤더
    PUT(mem_heap + (2*WSIZE), PACK(DSIZE, 1));  // 프롤로그 푸터
                                                // 실제 힙 확장하고 사용자 메모리 들어갈 곳
    PUT(mem_heap + (3*WSIZE), PACK(0, 1));      // 에필로그 헤더 (힙 영역 마무리)
    mem_heap += (2*WSIZE);
